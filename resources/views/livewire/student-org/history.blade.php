    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <x-mary-stat
                    title="Approved"
                    :description="'Success rate: ' . $this->stats['approved']['percentage'] . '%'"
                    :value="$this->stats['approved']['count']"
                    icon="s-check-circle"
                    color="text-success"/>
                ...
                <x-mary-stat
                    ...
                    :value="$this->stats['avgProcessingDays']"
                    icon="s-clock"
                    color="text-info"/>
            </div>
                        ...
                        <div class="space-y-3">
                            @foreach($this->monthlyActivity as $month)
                                <div class="grid grid-cols-[120px_1fr_80px] items-center gap-3">
                                    ...
                                    <span
                                        class="text-sm text-gray-600 text-right">{{ $month['count'] }} {{ Str::plural('event', $month['count']) }}</span>
                                </div>
                            @endforeach
                        </div>
                    @else
                ...
                <div class="space-y-6">
                    @forelse($this->tickets as $ticket)
                        @php
                            ...
                            $processingDays = $ticket->created_at && $ticket->updated_at
                                ? \Carbon\Carbon::parse($ticket->created_at)->diffInDays(\Carbon\Carbon::parse($ticket->updated_at))
                                : 0;
                        @endphp
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                        ...
                                        <div>
                                            ...
                                            <p class="font-medium">{{ $processingDays }} {{ Str::plural('day', $processingDays) }}</p>
                                        </div>
                                    </div>
                                </div>
