<div class="p-4 sm:p-6">
    ...
    <x-mary-card shadow class="border-none bg-slate-50/50 dark:bg-base-100/50 mb-6">
        <div class="flex flex-col lg:flex-row gap-4">
            {{-- Search --}}
            <div class="flex-1">
                <x-mary-input wire:model.live.debounce.300ms="search" placeholder="Search questions, answers..."
            ...
            @if ($search || $filterCategory || $filterStatus)
                <x-mary-button icon="o-x-mark" label="Clear" wire:click="clearFilters"
    ...
    <x-mary-card shadow class="border-none bg-slate-50/50 dark:bg-base-100/50">
                ...
                @foreach ($faqs as $faq)
                    <div data-faq-id="{{ $faq->id }}"
                                    ...
                                    @if ($faq->category)
                                        <div class="mt-2">
                                            <span
                                                ...
                                                <x-mary-icon name="o-tag" class="w-3 h-3" />
                                                {{ $faq->category }}
                                            </span>
                                        </div>
                                    @endif
                        ...
                        <div
                            ...
                            <x-mary-button size="xs" :icon="$faq->is_active ? 'o-eye' : 'o-eye-slash'"
        ...
        @else
            ...
            <div
                ...
                @if (!$search && !$filterCategory && !$filterStatus)
                    <x-mary-button icon="o-plus" label="Add First FAQ" class="btn-primary btn-sm mt-4"
                        link="{{ route('superadmin.faq.create') }}" wire:navigate />
                @endif
            </div>
        @endif
    ...
    @if ($deletingFaqId)
        <x-mary-modal wire:model="deleteModalOpen" title="Delete FAQ" subtitle="Confirm deletion" separator
            ...
            <x-slot:actions>
                <x-mary-button label="Cancel" @click="$wire.deleteModalOpen = false" class="btn-ghost" />
                <x-mary-button label="Delete FAQ" wire:click="confirmDelete" class="btn-error"
