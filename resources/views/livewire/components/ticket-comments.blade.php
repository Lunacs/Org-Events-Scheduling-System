<div class="bg-base-100 rounded-box shadow-lg p-6" x-data="{ isSubmitting: false }" x-init="$nextTick(() => { if (window.AvatarHelper) window.AvatarHelper.initAvatars(); })"
    ...
    @if ($ticket->areCommentsDisabled())
        <div class="mt-4 p-4 bg-base-200 rounded-lg border border-base-300">
            <div class="flex items-start gap-3">
                <svg class="w-5 h-5 text-base-content/70 mt-0.5 shrink-0" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                ...
                <div class="flex-1">
                    <p class="text-sm font-medium text-base-content/80">
                        @if ($ticket->status === 'completed')
                            Comments are disabled for completed events.
                        @else
                            Comments are disabled for events that have already passed.
                        @endif
                    </p>
                    <p class="text-xs text-base-content/60 mt-1">
                        You can still view previous comments above.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-3 mt-4">
            <div>
                <textarea wire:model.defer="comment"
                    class="textarea textarea-bordered w-full @error('comment') textarea-error @enderror" placeholder="Add a comment..."
