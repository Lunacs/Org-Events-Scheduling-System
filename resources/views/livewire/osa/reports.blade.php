...
<div>
        ...
        <div class="relative p-6 sm:p-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex items-center gap-2 relative z-10">
                    <x-ui.button wire:click.async="generateReport" class="btn-primary" icon="o-document-arrow-down"
                        spinner>
                        Generate Report
                    </x-ui.button>
                </div>
    ...
    @island(name: 'charts')
        <div class="mb-8" x-data="{
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <x-ui.button wire:click="loadChartData" class="btn-ghost btn-sm" icon="o-arrow-path"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-base-100 border border-base-300 rounded-box shadow-sm p-6">
                    <h3 class="font-semibold text-base-content mb-4 flex items-center gap-2">
                        <x-ui.icon name="o-chart-pie" class="w-5 h-5 text-secondary" />
                        Status Distribution
                    </h3>
                    <div class="h-64" wire:ignore>
                ...
                <div class="bg-base-100 border border-base-300 rounded-box shadow-sm p-6">
                    <h3 class="font-semibold text-base-content mb-4 flex items-center gap-2">
                        <x-ui.icon name="o-squares-2x2" class="w-5 h-5 text-accent" />
                        Events by Type
                    </h3>
                    <div class="h-64" wire:ignore>
                ...
                <div class="bg-base-100 border border-base-300 rounded-box shadow-sm p-6">
                    <h3 class="font-semibold text-base-content mb-4 flex items-center gap-2">
                        <x-ui.icon name="o-building-office" class="w-5 h-5 text-info" />
                        Top Organizations
                    </h3>
                    <div class="h-64" wire:ignore>
            ...
            <div class="bg-base-100 border border-base-300 rounded-box shadow-sm p-6">
                ...
                <div class="max-md:grid max-md:grid-cols-1 max-md:gap-4 space-y-6">
                    ...
                    <div>
                        ...
                        @endphp
                        <fieldset>
                            <legend class="block text-sm font-medium mb-2">Report Type</legend>
                            <div class="space-y-2">
                                @foreach ($reportTypeOptions as $option)
                                    <label
                                        class="flex items-start gap-3 p-2 rounded-lg cursor-pointer hover:bg-base-200 transition-colors">
                                        <input type="radio" class="radio radio-primary mt-1"
                                            wire:model.live="reportType" value="{{ $option['id'] }}"
                                            name="reportType" />
                                        <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <x-ui.input wire:model="dateFrom" label="From Date" type="date" />
                        <x-ui.input wire:model="dateTo" label="To Date" type="date" />
                    </div>

                    {{-- Organization Filter --}}
                    <x-ui.select wire:model="organizationFilter" label="Filter by Organization (Optional)"
                        placeholder="All Organizations" :options="$organizations" option-value="org_id"
                        option-label="org_name" />

                    {{-- Export Format --}}
                    <x-ui.select wire:model="exportFormat" label="Export Format" :options="[
                    ...
                    <div class="flex-row gap-2 sm:flex-col">
                        ...
                        <x-ui.button wire:click="clearFilters" class="btn-ghost" icon="o-x-mark">
                            Clear Filters
                        </x-ui.button>
                    </div>
                </div>
            ...
            <div class="bg-base-100 border border-base-300 rounded-box shadow-sm p-6">
                <div class="flex items-center gap-3 mb-4">
                    <div class="bg-success/10 p-2 rounded-full">
                        <x-ui.icon name="o-check-circle" class="w-5 h-5 text-success" />
                    </div>
            ...
            <div class="bg-base-100 border border-base-300 rounded-box shadow-sm p-6">
                <div class="flex items-center gap-3 mb-4">
                    <div class="bg-error/10 p-2 rounded-full">
                        <x-ui.icon name="o-x-circle" class="w-5 h-5 text-error" />
                    </div>
            ...
            <div class="bg-base-100 border border-base-300 rounded-box shadow-sm p-6">
                <div class="flex items-center gap-3 mb-4">
                    <div class="bg-warning/10 p-2 rounded-full">
                        <x-ui.icon name="o-clock" class="w-5 h-5 text-warning" />
                    </div>
    ...
    {{-- Report Preview --}}
    <div class="bg-base-100 border border-base-300 rounded-box shadow-sm">
        ...
        <div wire:loading.remove.delay wire:target="generateReport,reportType" class="p-6">
                        ...
                        <tbody>
                            @foreach ($reportData->take(10) as $ticket)
                                ...
                                <tr>
                                    ...
                                    <td>{{ $ticket->updated_at?->format('M d, Y') ?? 'TBD' }}</td>
                                    <td>
                                        <x-ui.badge value="For Revision" class="badge-error text-white" />
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
            ...
            @else
                ...
                <div class="text-center py-12">
                    <x-ui.icon name="o-document-chart-bar" class="w-16 h-16 text-base-content/30 mx-auto mb-4" />
