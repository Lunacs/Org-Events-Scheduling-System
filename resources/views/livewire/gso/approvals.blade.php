                        ...
                        <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                            ...
                            <div class="flex flex-col space-y-2 ml-4">
                                <div class="flex space-x-2">
                                    <x-mary-button wire:click="confirmAction({{ $approval['id'] }}, 'approve')"
                                                   label="Approve" icon="s-check" class="btn-sm btn-success" />
                                    <x-mary-button wire:click="confirmAction({{ $approval['id'] }}, 'reject')"
                                                   label="Reject" icon="s-x-mark" class="btn-sm btn-error" />
                                </div>

                                @if ($approval['ticket_id'])
                                    <x-mary-button label="Details" icon="s-eye"
                                                   class="btn-sm btn-outline btn-emerald"
                                                   link="{{ route('gso.ticket-details', ['ticket' => $approval['ticket_id']]) }}"
                                                   wire:navigate />
                                @else
                                    <x-mary-button label="Details" icon="s-eye" class="btn-sm btn-outline" disabled />
                                @endif
