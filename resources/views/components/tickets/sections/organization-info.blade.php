...
<div class="bg-base-100 rounded-box shadow-lg p-4 md:p-6 overflow-hidden">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-4">
        ...
        @if ($ticket->user->studentOrganization)
            <img src="{{ $ticket->user->studentOrganization->logo_url }}"
                alt="{{ $ticket->user->studentOrganization->org_name }} logo" class="w-16 h-16 md:w-20 md:h-20 object-cover rounded-lg flex-shrink-0">
        @endif
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 overflow-hidden">
        ...
        <div class="min-w-0">
            ...
            <p class="text-base-content break-words">{{ $ticket->user?->name }}</p>
        </div>

        <div class="min-w-0">
            <label class="text-sm font-medium text-base-content/70">Proponent Position</label>
            <p class="text-base-content break-words">{{ $ticket->user?->position?->position_name ?? 'N/A' }}</p>
        </div>

        <div class="min-w-0">
            <label class="text-sm font-medium text-base-content/70">Contact Email</label>
            <p class="text-base-content break-all">{{ $ticket->user?->email }}</p>
        </div>
