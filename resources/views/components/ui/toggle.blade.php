@props([
    'label' => null,
    'hint' => null,
    'right' => false,
    'id' => null,
])

@php
    // Resolve the bound Livewire field so the inline error can be driven from it.
    $field = $attributes->whereStartsWith('wire:model')->first();

    // Ensure a stable id so the label can be associated via for/id.
    $toggleId = $id ?? 'ui-toggle-' . \Illuminate\Support\Str::random(8);
@endphp

<div>
    <label for="{{ $toggleId }}" @class([
        'flex gap-3 items-center cursor-pointer',
        'justify-between' => $right,
        'items-start' => $hint,
    ])>
        {{-- TOGGLE: forwards wire:model, required verbatim --}}
        <input id="{{ $toggleId }}" type="checkbox"
            {{ $attributes->whereDoesntStartWith('id')->class(['toggle', 'order-2' => $right]) }} />

        {{-- LABEL --}}
        <div @class(['order-1' => $right])>
            @if ($label)
                <span class="text-sm font-medium">
                    {{ $label }}
                    @if ($attributes->get('required'))
                        <span class="text-error">*</span>
                    @endif
                </span>
            @endif

            {{-- HINT --}}
            @if ($hint)
                <p class="text-xs opacity-60">{{ $hint }}</p>
            @endif
        </div>
    </label>

    {{-- INLINE VALIDATION ERROR --}}
    @if ($field)
        @error($field)
            <x-ui.input-error :messages="$message" class="mt-1" />
        @enderror
    @endif
</div>
